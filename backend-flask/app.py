...
# --------------------------------------------------------------------------------
# NEW VULNERABILITIES ADDED BELOW
# --------------------------------------------------------------------------------
...
def run_code():
    ...
    Evaluating code directly from user input with `eval`.
    """
    code_to_run = request.json.get('code', '')
    try:
        # Using eval on raw user input => extremely dangerous
        result = eval(code_to_run)  # Vulnerability: Remote code execution
